    ...
    <div className="tc-card" style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 20 }}>
      ...
      {people.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
          {/* ⚠️ O clique no nome abre o PAINEL DA PESSOA (cursos concluídos e
              vídeos assistidos no período) em vez de sair para a ficha — a ficha
              fica a um link dentro do painel. Ver `PainelDaPessoa.tsx`. */}
          {people.map((p, i) => (
            <div key={p.id} className="tc-row" role="button" tabIndex={0}
              onClick={() => abrirPessoa('classroom', p.id)}
              onKeyDown={(e) => { if (e.key === 'Enter') abrirPessoa('classroom', p.id) }}
              title="Ver os cursos concluídos e os vídeos assistidos no período"
              style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', borderRadius: 8, padding: 5, margin: '-1px -5px' }}>
