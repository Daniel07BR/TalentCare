...
/* ============================================================
   O CORPO da página de Turnover, sem hooks — para servir aos DOIS lados.

   ⚠️ A página `/turnover` é montada no SERVIDOR (lê o banco direto). O detalhe
   do setor, dentro do relatório, é CLIENTE (abre numa janela). Um componente
   sem hook e sem 'use client' roda nos dois: o servidor passa o `vm` que montou,
   a janela passa o `vm` do diretório recortado. A tela é uma só.
   ============================================================ */

export function TurnoverVisao({ vm, setor = null, taxa }: {
  vm: ReturnType<typeof turnoverVM>
  setor?: SetorRecorte | null
  /** A rotatividade do setor pela régua do relatório — só no modo setor. */
  ...
  return (
    <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: setor ? '1fr' : '1.5fr 1fr', gap: 16, marginBottom: 16 }}>
      ...
      {/* Relatório por departamento — num setor só, é uma linha: sai */}
      {!setor && (
