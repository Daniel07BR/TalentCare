...
/* ============================================================
   A ESCOLARIDADE POR DEPARTAMENTO — o corpo da página `/formacao`, que também
   abre na JANELA (11/09/2026, pedido do dono: "no relatório a escolaridade não
   está dando clique para ver a janela com o resumo da escolaridade por usuário").

   ⚠️ Como os resumos dos sistemas: a rota só renderiza isto, e a janela o
   renderiza dentro de `RecorteDoSetor` (o setor) ou de `EmJanela` (a casa). No
   setor, o dataset já é só o setor — o bloco "Empresa" sairia repetindo o cartão
   do próprio setor, então some.

   ⚠️ Retrato de HOJE: a formação não acompanha o filtro de período.
   ============================================================ */
export default function FormacaoResumo() {
    ...
    <div className="tc-anim" style={emJanela ? undefined : { maxWidth: 1280, margin: '0 auto' }}>
      ...
      {emJanela && (
        <div style={{ fontSize: 12, color: 'var(--text-dim)', marginBottom: 14 }}>
          Retrato de hoje — a formação não acompanha o filtro de período. Quem tem mais de uma pós aparece com cada uma.
        </div>
      )}

      {/* Resumo geral — no setor ele repetiria o cartão do próprio setor */}
      {!setor && (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        {vm.byDept.map((d) => (
          <div key={d.id} className="tc-card" style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 18 }}>
            <div className={setor ? undefined : 'tc-row'} onClick={setor ? undefined : () => router.push(`/departamentos/${d.id}`)} style={{ display: 'flex', alignItems: 'center', gap: 16, cursor: setor ? 'default' : 'pointer', borderRadius: 8, padding: '4px 6px', margin: '-4px -6px 14px' }}>
