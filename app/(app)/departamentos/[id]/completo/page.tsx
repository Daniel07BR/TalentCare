...
import { useDetalhe, JanelaDetalhe, BotaoDetalhe, type ChaveDetalhe } from '../../../_visao/Detalhe'
...
export default function DepartamentoDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (estado === 'negado') {
    return (
      <div className="tc-anim" style={{ width: '100%' }}>
        <button onClick={() => router.push(`/departamentos/${id}`)} style={voltar}>‹ Voltar à visão geral do setor</button>
  ...
  if (!vm) {
    return (
      <div className="tc-anim" style={{ width: '100%' }}>
        <button onClick={() => router.push(`/departamentos/${id}`)} style={{ background: 'transparent', border: 'none', color: 'var(--text-dim)', cursor: 'pointer', fontFamily: 'inherit', fontSize: 13, fontWeight: 500, padding: 0, marginBottom: 18 }}>‹ Voltar à visão geral do setor</button>
    ...
    <div className="tc-anim" style={{ width: '100%' }}>
      ...
      */}
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', marginBottom: 16 }}>
        <div>
          {/* ⚠️ Desde 11/09/2026 este é o relatório COMPLETO: a porta do setor é a
              visão geral (`../page.tsx`, o desenho da imagem conceito). Aqui fica o
              que ela não traz — a tabela de pessoas com nota e busca, a avaliação
              por critério, a tendência, os cartões de cada sistema. */}
          <div style={{ fontSize: 12, color: 'var(--text-dim)', fontWeight: 500, marginBottom: 4 }}>Relatório completo do setor</div>
