...
export function Cartao({ titulo, sub, Icone, acao, children, className, style }: {
  titulo?: string; sub?: React.ReactNode; Icone?: LucideIcon; acao?: React.ReactNode
    ...
    <section className={`${s.cartao} ${className ?? ''}`} style={style}>
      {titulo && (
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 14 }}>
          {Icone && <Icone size={18} color="var(--n-blue)" style={{ flex: 'none', marginTop: 1 }} />}
...
export function Tile({ Icone, tom, valor, rotulo, nota, className, alto = false }: {
  Icone: LucideIcon; tom: Tom; valor: React.ReactNode; rotulo: string
  nota?: React.ReactNode; className?: string; alto?: boolean
}) {
  return (
    <div className={`${s.cartao} ${className ?? ''}`}
      style={{ padding: 16, display: 'flex', flexDirection: alto ? 'column' : 'row', alignItems: alto ? 'flex-start' : 'center', gap: 14 }}>
...
export function Mini({ valor, rotulo, tom }: { valor: React.ReactNode; rotulo: React.ReactNode; tom?: Tom }) {
  return (
    <div style={{ background: 'var(--n-card-2)', border: '1px solid var(--n-border-2)', borderRadius: 10, padding: '11px 12px', minWidth: 0 }}>
