...
function diasDoIntervalo(de: string, ate: string): string[] {
  ...
  // Teto de segurança: um intervalo absurdo não pode travar o navegador.
  while (t <= fim && out.length < 400) {
...
export function ConclusoesPorDia({ m }: { m: EntregasMetrics }) {
  ...
  const dias = diasDoIntervalo(m.fromDay, m.toDay)
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10, flexWrap: 'wrap', marginBottom: 14 }}>
        <div>
          ...
          <div style={{ fontSize: 11.5, color: 'var(--text-dim)', marginTop: 2 }}>
            {totalDias} {totalDias === 1 ? 'dia' : 'dias'} com serviço em {dias.length} do intervalo · pico de{' '}
