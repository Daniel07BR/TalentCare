...
// sync incremental das fontes (Rádio + WhatsApp + ClassRoom + Consultoria + HelpDesk)
...
export async function POST() {
  ...
  const [radio, whatsapp, classroom, consultoria, helpdesk] = await Promise.allSettled([
    syncRadio(),
    syncWhatsapp(),
    syncClassroom(),
    syncConsultoria(),
  ...
  return NextResponse.json({ ok: true, radio: val(radio), whatsapp: val(whatsapp), classroom: val(classroom), consultoria: val(consultoria), helpdesk: val(helpdesk) })
}
