/* ============================================================
   TalentCare — Turnover & Headcount. Porte de turnoverVM.
   ============================================================ */
import { buildData, geomLine } from './data'

export function turnoverVM() {
  const D = buildData()
  ...
  const byDept = [...D.departments].sort((a, b) => b.turnover - a.turnover).map((d) => ({
  ...
  return {
    ...
    motivos, byDept, line: tl.line, area: tl.area, rate: 8.4, headcount: D.employees.filter((e) => e.status !== 'Desligado').length,
  }
}
