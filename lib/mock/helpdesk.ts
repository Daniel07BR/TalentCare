...
export type HelpdeskTotals = { opened: number; resolved: number; resolvedSeconds: number }
...
export type HelpdeskPerson = {
  ...
  opened: number; resolved: number; resolvedSeconds: number; tempoMedio: string
}
...
export function helpdeskVM(data: TalentData, period?: HelpdeskUsage) {
  ...
  const stat = (e: TalentData['employees'][number]): HelpdeskStat => {
    if (period) {
      ...
      return { opened: p?.opened ?? 0, resolved: p?.resolved ?? 0, resolvedSeconds: p?.resolvedSeconds ?? 0 }
    }
    return e.helpdesk
  }
  ...
  const person = (e: TalentData['employees'][number]): HelpdeskPerson => {
    ...
    return {
      ...
      opened: s.opened, resolved: s.resolved, resolvedSeconds: s.resolvedSeconds,
      tempoMedio: fmtDur(s.resolved ? Math.round(s.resolvedSeconds / s.resolved) : 0),
    }
  }
  ...
  const totals: HelpdeskTotals = byUser.reduce(
    (a, p) => ({ opened: a.opened + p.opened, resolved: a.resolved + p.resolved, resolvedSeconds: a.resolvedSeconds + p.resolvedSeconds }),
    { opened: 0, resolved: 0, resolvedSeconds: 0 },
  )
  const tempoMedioGeral = fmtDur(totals.resolved ? Math.round(totals.resolvedSeconds / totals.resolved) : 0)
  ...
  for (const e of data.employees) {
    const s = stat(e)
    if (s.opened <= 0 && s.resolved <= 0) continue
